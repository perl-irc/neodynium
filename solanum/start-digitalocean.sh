#!/bin/sh
# ABOUTME: Solanum IRCd startup script for DigitalOcean App Platform
# ABOUTME: Handles secure password generation, template processing, and IRC server startup without Tailscale

set -e

echo "Starting Solanum on DigitalOcean App Platform..."
...
if [ ! -f "$SSL_CERT" ] || [ ! -f "$SSL_KEY" ]; then
    ...
    openssl req -x509 -nodes -newkey rsa:4096 \
        ...
        -out "$SSL_CERT" \
        -days 365 \
        -subj "/C=US/ST=State/L=City/O=MagNET IRC Network/CN=${SERVER_NAME}.ondigitalocean.app"
fi
...
echo "Starting Solanum in foreground mode..."
...
check_solanum() {
    if ! pgrep -f "/opt/solanum/bin/solanum" > /dev/null; then
        echo "Solanum process died, exiting"
        return 1
    fi
}
